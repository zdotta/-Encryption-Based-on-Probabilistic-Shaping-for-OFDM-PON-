...
figure;
hold on;
seq_types = {'logistic', 'random', 'hadamard','none'};  % 添加你支持的类型
colors = lines(length(seq_types));
legend_entries = cell(1, length(seq_types));
tx_phase_group_num = 3; % 设定固定的相位组数，例如3组

for idx = 1:length(seq_types)
    tx_seq_type = seq_types{idx};
    if strcmp(tx_seq_type,'none')
        ...
        % SLM处理
        [tx_time_signal, ~, ~] = slm_papr_reduction(tx_data_with_pilots, tx_N_FFT, tx_phase_perturbations);
    end
    % 计算PAPR
    ...
    % 绘图
    semilogy(papr_vals, ccdf_vals, 'LineWidth', 2, 'Color', colors(idx, :));
    legend_entries{idx} = sprintf('Sequence: %s', tx_seq_type);
end
...
title(sprintf('CCDF for Different Chaotic Sequence Types(group-num = %d)',tx_phase_group_num));
