...
tx_CP_ratio = 1/4;
tx_mod_order = 16;
tx_used_subcarriers = 160;
tx_num_symbols = 10;
tx_pilot_interval = 10;
...
scatterplot(tx_qam_symbols(:));
title(sprintf('16QAM后 置乱前 信号星座图'));
...
scatterplot(tx_scrambled_data(:));
title(sprintf('置乱后 加导频前 信号星座图'));
...
scatterplot(tx_data_with_pilots(:))
title(sprintf('加导频后 slm前星座图'));
...
scatterplot(tx_freq_signal(:));
title(sprintf('slm后星座图'));
...
switch channel
    case  'ideal'
% % 理想信道（直接使用发送信号）
rx_received_signal = OFDM;
    case 'noise'
    % 加噪信道
    rx_received_signal = awgn(OFDM,30);
    case 'fiber'
% % 光纤信道传输
...
% 串并转换
rx_parallel_signal = serial_to_parallel(rx_truncated_signal,rx_used_subcarriers,rx_mod_order);
...
scatterplot(rx_valid_freq(:));
title(sprintf('%s信道 信道估计前信号星座图',channel));
% 5. SLM解调
% 使用相同的函数生成相位扰动
...
scatterplot(rx_demodslm_freq(:));
title(sprintf('%s信道 slm-demod信号星座图',channel));
% 6. 信道均衡
% 应用信道均衡
...
% 绘制均衡后信号的星座图
scatterplot(rx_valid_freq_H(:));
title(sprintf('%s信道 信道估计与均衡化后信号星座图',channel));
...
scatterplot(rx_valid_freq_H(:));
title(sprintf('%s信道 删除导频后的星座图',channel));
